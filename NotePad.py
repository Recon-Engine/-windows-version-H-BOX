...
class Notepad:
   ...
   def __init__(self,**kwargs):
      ...
      self._root.title("HACKERS BOX - Notepad")
      photo = PhotoImage(file=r"icon.png")
      self._root.iconphoto(False, photo)
      ...
      self.FileMenu.add_command(label="New File", command=lambda:self.new())
      self.FileMenu.add_command(label="Open File", command=lambda:self.open())
      self.FileMenu.add_command(label="Save File", command=lambda:self.save())
   ...
   def about(self):
      showinfo("About Notepad","This is a prototype of a Notepad. \nFor any quarry contact us on instagram @recon.engine.official")
